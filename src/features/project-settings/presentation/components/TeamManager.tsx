...
export function TeamManager({ workspaceId, members, invitations }: Props) {
  ...
  const changeStatus = async (member: ProjectMember, status: "active" | "inactive") => {
    try {
      ...
      toast.success(status === "active" ? "Miembro reactivado." : "Miembro desactivado.");
    } catch {
      /* handled */
    }
  };
    ...
    <Card className="flex flex-col gap-4 p-5">
      <div className="flex items-start justify-between gap-3">
        <SectionHeader
          ...
          description="Invita, asigna roles y gestiona el estado de los miembros."
        />
        <button
          type="button"
              ...
              {invitations.map((inv) => (
                <tr key={inv.id} className="border-b border-line/60 align-middle opacity-80">
                  <td className="py-3 pr-3">
                    ...
                    <div className="text-[12px] text-ink-3">Invitación pendiente</div>
                  </td>
                  ...
                  <td className="py-3 text-right text-[11px] text-ink-3">esperando aceptar</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="text-[11.5px] text-ink-3">
        Los roles <strong>{PROJECT_ROLES.filter((r) => r.isLeadership).map((r) => r.name).join(", ")}</strong> se
        consideran liderazgo. Debe existir siempre al menos un líder activo.
      </p>
      ...
      >
        <p className="text-sm text-ink-2">
          ¿Seguro que deseas eliminar a <strong className="text-ink">{confirmRemove?.name}</strong> del
          proyecto? Esta acción se puede revertir volviendo a invitar.
        </p>
